...
export default {
    ...
    methods: {
        saveSlide() {
            ...
            } else if (!this.ad_file_url) {
                this.$message.error('Your must upload an image file for this slide.')
            } else {
                const annData = new FormData();
                annData.append("title", this.ad_title)
                annData.append("body_text", this.ad_content)
                annData.append("order", this.ad_order)
                annData.append("file_path", this.ad_file)
                            
                axios.post(`/api/pcr/ads`, annData,
                    {
                    headers: { Authorization: "Bearer " + this.token }
                })
                .then(response => {
                    // console.log(response)
                    this.$message.success('Slide successfully added.')
...
.ad-sidebar .el-card__header {
    font-weight:bold;
    color:#174A84;
}
.ad-featured-upload,
.ad-featured-upload .el-upload,
.ad-featured-upload img {
    width:100%;
}
.blank-featured-img {
