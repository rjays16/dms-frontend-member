    ...
    <div>
        <div class="content-wrapper">
            <div class="admin-page-title bg-white p-3">
                ...
                <span> / Tags / Update User Tag</span>
            </div>            
...
export default {
    ...
    methods: {
        getTag() {
            axios.get(this.apiDomain + `/api/tags/${this.user_id}`, {
                headers: { Authorization: "Bearer " + this.token }
            })
            .then(response => {
                // // console.log(response)
                if(response.data) {
                    this.tag_name = response.data.data.tag_name
                }
            })
            .catch(error => {
                // // console.log(error)
        ...
        saveUser() {
            ...
            if (!this.tag_name) {
                ...
                axios.put(`/api/tags/update/${this.user_id}`, {
                ...
                .then(response => {
                    // // console.log(response)
                    this.$message.success('Tag successfully updated.')
                    this.$router.push('/admin/tags')
                })
                .catch(error => {
                    this.$message.error('Tag could not be updated. Please try again.')
                    // // console.log(error)
                })
