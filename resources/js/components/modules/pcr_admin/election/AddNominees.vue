                    ...
                    <div class="table-responsive">
                         <el-table
                                ...
                                <el-table-column
                                    ...
                                    label="Actions">
                                    <template slot-scope="scope">
                                        <div v-if="scope.row.is_approved === 1">
                                        <el-button size="mini" @click="review(scope.row.id)" type="primary" icon="el-icon-success">Nominate</el-button>
                                        </div>
                                        <div v-else>
...
export default {
    ...
    computed: {
        ...
        filterApprovedMembers() {
            if (this.members.data) {
                let members = this.members.data
                return members.filter(data => data.is_approved === 1)   
            }
        }
    ...
    methods: {
        ...
        getMembers() {
            axios.get(this.apiDomain + '/api/election/members/'+this.period_id+'?page=1', {
                headers: { Authorization: "Bearer " + this.token }
            })
			.then(res => {
				if(res.data) {
        ...
        getSearchResults() {
            ...
            axios.get(this.apiDomain + `/api/election/search?keyword=${this.searchTerm}&id=${id}`, {
                headers: { "Authorization": "Bearer " + this.token }
            })
			.then(res => {
				if(res.data) {
        ...
        nominate() {
            $('.nomite-btn').attr('disabled','disabled');
             this.nominateConfirmationTab = false 
                this.$v.$touch();
                if(!this.$v.$invalid){
                    axios.post(this.apiDomain + '/api/election/nominate', {
                        ...
                        .catch(error => {
                            if(error.response.data.status_code == 400) {
                                ...
                                this.$message.error('Failed to nominate. Please contact the site admin.')
                                // // console.log(error)
                            }
                        })
                } else {
