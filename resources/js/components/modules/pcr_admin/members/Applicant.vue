<template>
    <div>
        <div class="content-wrapper">
            <div class="admin-page-title bg-white p-3">
                ...
                <span> / Members / Applicant</span>
            </div>              
            <div class="container-fluid pb-5">
                <div class="admin-conent bg-white my-5 mx-3 p-4">
                    <h1>Applicant Members <span class="member-count" v-if="totalMembers">{{ totalMembers }}</span></h1>
                    <div class="row mb-4">
                    ...
                    <div class="table-responsive">
                        <el-table
                            ...
                            <el-table-column
                                ...
                                label="Deposit Slip">
                                <template slot-scope="scope">
                                    <el-button plain class="my-1 mx-0" size="mini" :loading="is_approving" :disabled="is_approving" @click="view(scope.row.id)" type="warning" icon="el-icon-search">View</el-button>
                                </template>
                            </el-table-column>
                            <el-table-column
                                prop="actions"
                                ...
                                <template slot-scope="scope">
                                    <!-- <el-button plain class="my-1 mx-0" size="mini" :loading="is_approving" :disabled="is_approving" @click="view(scope.row.id)" type="warning" icon="el-icon-search">View Deposit Deposit Slips</el-button> -->
                                    ...
                                    <el-button plain class="my-1 mx-0" size="mini" :loading="is_approving" :disabled="is_approving" @click="deleteConfirmation(scope.row.id)" type="danger" icon="el-icon-close">Reject</el-button>                                    
                                </template>
                            </el-table-column>
                        </el-table>

                        <div class="block text-center mt-5" v-if="!showSearchRes && totalMembers">
    ...
    <el-dialog :visible.sync="applyReasonConfirmationTab">
        <div class="text-center">Are you sure you want to reject application for this user?</div>
        <div class="text-center mt-3">
            <el-button :loading="is_approving" :disabled="is_approving" @click="applyReason()" type="success">Yes</el-button>
            <el-button @click="cancelApplyReason" type="danger">No</el-button>
        </div>
    </el-dialog>
    ...
    <el-dialog :visible.sync="deleteConfirmationTab">
        ...
        <!-- <div class="text-center mb-3 ml-2">Are you sure you want to reject the pending application for this user?</div> -->
            <div class="form-group">
                <div class="row">
                ...
                <div class="col-7">
                    <el-input class="pr-4" v-model="$v.reasonInput.reason.$model" type="text" />
                </div>
                    <div class="" v-if="$v.reasonInput.reason.$dirty">
                        <div class="note color-FF2C2C mt-2" v-if="!$v.reasonInput.reason.required">*Required Field</div>
                        <!-- <div class="note color-FF2C2C mt-3" v-if="!$v.reasonInput.reason.maxLength">*Max of {{ $v.reasonInput.reason.$params.maxLength.max }} char.</div> -->
                    </div>
                </div>
		</div>
        
        <div class="text-center mt-5">
            <el-button :loading="is_approving" :disabled="!$v.reasonInput.reason.$model" @click="recheckReasonInputs()" type="danger" icon="el-icon-delete">Reject</el-button>
    ...
    <el-dialog class="member-review-dialog" title="Review Member Application" :visible.sync="dialogReviewTab">
        <table width="100%" cellspacing="0" cellpadding="7">
            ...
            <tr v-if="currentReview.prc_upload">
                <td><span class="font-weight-bold">PRC ID: </span></td>
                <td><el-button @click="openImage(currentReview.prc_upload)" type="primary">View</el-button></td>
            </tr>
            <tr>
...
export default {
    data() {
        return {
            ...
            applyReasonConfirmationTab: false,
            ...
            manual_token: "2071|kPPxRSDR8C6UMjIjCciW4yvM2xdJi3XE3haN5YHm",
        }
    },
    validations: {
        reasonInput: {
    ...
    methods: {
        recheckReasonInputs() {
            ...
            if(this.$v.reasonInput.$invalid) {
                ...
                // this.saveReason()
                this.applyReasonConfirmation()
            }
        },
        saveReason() {
            // // console.log(this.formCollection)

            this.$confirm('This Rejet Reason will be added for this member. Continue?', 'Warning', {
				confirmButtonText: 'OK',
				cancelButtonText: 'Cancel',
				type: 'warning'
			}).then(() => {
				axios.post(this.apiDomain + "/api/members", this.reasonInput, {
                    headers: {
        ...
        getSearchResults() {
            ...
            axios.get(this.apiDomain + '/api/admin/members/applicant', {
				params: {
                    is_search: true,
        ...
        approvalConfirmation(id) {
            ...
            console.log('id:', this.forApproveID)
        },
        deleteConfirmation(id) {
            this.deleteConfirmationTab = true
            this.forDeleteID = id
        },  
        applyReasonConfirmation(id) {
            this.applyReasonConfirmationTab = true
            // this.forApplyReasonID = id
        },       
        cancelApprove() {
        ...
        cancelApplyReason() {
            this.applyReasonConfirmationTab = false
            // this.forApplyReasonID = ''
        },
        cancelDelete() {
            this.deleteConfirmationTab = false
            this.forDeleteID = ''
        ...
        approve() {
            ...
            // axios.put(this.apiDomain + `/api/admin/members/pending/${id}/approve`, {
            //     headers: { "Authorization": "Bearer " + this.token }
            // })
        ...
        applyReason() {
            ...
            axios.post(this.apiDomain + `/api/admin/members/applicant/${id}`, this.reasonInput, {
                headers: {
        ...
        deleteReg() {
            ...
            this.is_approving = true
            axios.put(this.apiDomain + '/api/member/decline/' + id, {
                headers: { "Authorization": "Bearer " + this.token }
            }).then(res => {
                if(res.data) {
                    this.getApplicantMembers();
                    ...
                    this.dialogReviewTab = false
                    this.$notify({
                        message: 'Selected application has been rejected.',
                        type: 'success',
                        customClass: 'pcr-login-error',
                        position: 'bottom-right'
                    })                                     
        ...
        approveReview(id) {
            ...
            axios.put(this.apiDomain + `/admin/registration/pending/${id}/approve`)
            .then(res => {
                this.getApplicantMembers();
