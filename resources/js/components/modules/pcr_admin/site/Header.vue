    ...
    methods: {
			...
			.then(res => {
				...
				if(res.data.status_code === 200) {
                    if (res.data.data.length > 0) {
                        ...
                        this.header_file_url = this.apiDomain + '/' + res.data.data[0].header_logo_img.replace("public", "storage")
        ...
        saveHeader() {
            if (!this.header_file) {
                ...
                axios.post(this.apiDomain + `/api/headerlogo`, headerData,
                    {
                    headers: { Authorization: "Bearer " + this.token }
                })
                .then(response => {
        ...
        editHeader() {
             axios.put(this.apiDomain + `/api/headerlogo/update/${this.header_id}`, {
            ...
            .then(response => {
                ...
                if (this.header_file) {
                    ...
                    axios.post(this.apiDomain + `/api/headerlogo/upload`, headerData,
                        {
                        headers: { Authorization: "Bearer " + this.token }
                    })
                    .then(response => {
