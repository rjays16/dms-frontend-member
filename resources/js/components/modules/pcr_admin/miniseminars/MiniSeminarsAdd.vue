    ...
    <div>
        <div class="content-wrapper">
            <div class="admin-page-title bg-white p-3">
                ...
                <span> / Mini-Sessions / Add Mini-Session</span>
            </div>
    ...
    methods: {
        ...
        saveSesion1() {
            ...
            if (!this.topic) {
                ...
                axios.post(`/api/minisession/create`, {
                ...
                .then(response => {
                    ...
                    this.$message.success('Mini-Session successfully added.')
                    this.$router.push('/admin/mini-Sessions')
                })
                .catch(error => {
                    if (error.response.data.status_code === 401) {
                        this.$message({
                            message: 'Mini-session could not be added. Please make sure you have provided a valid Zoom Email Account, API, and Secret.',
                            type: 'error',
                            duration: 10000
                        })
                    } else {
                        this.$message({
                            message: 'You cannot book another mini-session on the same day/time with the same zoom account and zoom email.',
                            type: 'error',
                            duration: 10000
                        })
                    }
        ...
        saveSesion2() {
            ...
            axios.post(`/api/minisession/session/${this.session.room_id}`, {
                topic: this.topic,
                type: 2,
                start_time: this.start_date + 'T' + this.start_time,
                duration: this.duration,
                schedule_for: this.session.email,
                timezone: 'Asia/Singapore',
                password: finpw,
                settings: {
                    host_video: true,
