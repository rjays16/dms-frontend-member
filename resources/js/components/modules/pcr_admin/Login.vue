              ...
              <div class="d-flex justify-content-center">
                <el-card class="box-card login-card">
                  <div slot="header" class="clearfix text-center">
					<img :src="logo_img" class="mb-3" style="max-height:100px;" />
                    <h1>ADMIN LOGIN</h1>
                  </div>
  ...
  export default {
    ...
    data() {
        return {
			...
			keep_signed: false,
			signing_in: false,
			//apiDomain: process.env.MIX_API_SERVER_DOMAIN,
	...
	methods: {
		submit() {
			...
			axios.post(this.apiDomain + '/api/auth/admin/login', data)
			.then(response => {
				...
				if (response.data.role === 'admin' || response.data.role === 'super_admin') {
					...
					location.href='/admin/members/applicant'					
