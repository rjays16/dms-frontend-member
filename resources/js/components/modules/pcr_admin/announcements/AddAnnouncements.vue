    ...
    <div>
        <div class="content-wrapper">
            <div class="admin-page-title bg-white p-3">
                ...
                <span> / Announcements / Add Announcement</span>
            </div>            
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-3">
                                <div class="ann-sidebar">
                                    <el-button @click="saveAnn" class="mb-4" type="warning">Publish</el-button>
...
export default {
    ...
    methods: {
        saveAnn() {
            ...
            } else if (this.ann_content.length > 1000000) {
                this.$message.error('Your announcement text is too long. Please reduce the number of characters')
            } else {
                const annData = new FormData();
                annData.append("title", this.ann_title)
                annData.append("body_text", this.ann_content)
                annData.append("category", this.ann_categ)
                annData.append("file_path", this.ann_file)
                            
                axios.post(this.apiDomain + `/api/pcr/announcement`, annData,
                    {
                    headers: { Authorization: "Bearer " + this.token }
                })
                .then(response => {
                    // console.log(response)
                    this.$message.success('Announcement successfully added.')
