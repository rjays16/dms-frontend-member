							...
							<div class="row">
								...
								<div class="col-md-6 col-sm-12">
									<div class="sec-personal-info">
										...
										<el-input class="mb-3" placeholder="Contact Number" v-model="$v.step1.contact_num.$model" maxlength="11"/>
									</div>
									<!-- ================= -->
									<!-- <div v-if="$v.step1.reason.$dirty">
											<div class="invalid-field" v-if="!$v.step1.reason.required">
												Required
											</div>
										</div>
										<el-input class="mb-3 name-uppercase" placeholder="Reason" v-model="$v.step1.reason.$model" /> -->

										
									<div class="deposit-slip-info">
										<p class="mb-0 font-weight-bold">
											Deposit Slip
										</p>
										<Label> Have a deposit slip?</Label>
										<el-radio-group class="pt-2 pl-2" v-model="deposit_slip" @change="showCollectionFeesList(false)">
											<el-radio :label=true> Yes </el-radio>
											<el-radio :label=false> No </el-radio>
										</el-radio-group>
										<p v-if="show_collection_list===true"> For what collection fee? <small><i>Note: Make sure exact amount was being paid for specific fee.</i></small></p>
										<div v-if="show_collection_list===true">
											<ul>
												<li v-for="item in collection_list" :key="item.id">
													<div class="row mb-2">
														<div class="col-lg-4 col-sm-12 col-md-12 text-left">
															{{ item.description }}
														</div>
														<div class="col-lg-5 col-sm-12 col-md-12 text-left">
															<small v-if='item.file_upload && item.file_upload!==""? item.file_upload.name:""'>{{ item.file_upload.name}}
															<el-button class="p-1" type="danger" plain style="font-size:12px" @click="removeDepositSlip(item)">
																Remove <i class="fa fa-close"></i>
															</el-button>
															</small>
														</div>
														<div class="col-lg-3 col-sm-12 col-md-12 text-left">
															<el-upload
																class="avatar-uploader"
																action="/"
																:disabled="item.file_upload && item.file_upload!=='' ? true:false"
																accept="image/png, image/jpeg, image/jpg"
																:auto-upload="false"
																:show-file-list="false"
																:on-change="addFileToUploadDepositList">
																<el-button
																	class="upload-btn p-2"
																	type="primary"
																	round
																	:disabled="item.file_upload && item.file_upload!=='' ? true:false"
																	@click="saveDepositSlip(item)"
																	size="small">
																	Upload Deposit Slip
																</el-button>
															</el-upload>
								...
								<div class="col-md-6 col-sm-12">
									...
									<div class="sec-photo text-left mt-3 mb-4">
										...
										<div v-if="$v.step3.imageUrl3final.$dirty">
								...
								<div class="col-12 sec-submit mt-4 text-center">
									<el-button class="btn-submit" type="primary" :disabled="!$v.step3.conf_password.sameAsPassword" round @click="submit">
										SUBMIT
									</el-button>
...
import { required, requiredIf, integer, email, sameAs } from "vuelidate/lib/validators/";
	export default {
		...
		data() {
			return {
				...
				edu: {
					...
					// spec_graduation: "",
					specialty_society: "",
					specialty_society_graduation: null,
					subspecialty: "",
					subspecialty_society: "",
					subspecialty_society_induction: null,
					master_education: "",
		...
		methods: {
			...
			checkuser() {
				axios.get(this.apiDomain + "/api/checkuser?email=" + this.email + "&prc_number=" + this.prc_num)
				.then((res) => {
					if (
			...
			submit() {
				...
				if(!this.$v.$invalid) {
					...
					for( var i = 0; i < this.collection_list.length; i++ ){
						let file = this.collection_list[i]['file_upload'];
						memData.append('collection_list[' + this.collection_list[i].id + ']', file);
					};
					...
					if(this.isCeritified) {
						...
						.catch((error) => {
							console.log(error);
							let message = 'There was an error saving your application. Please make sure that all required fields are filled in. Else, contact website admin.'
							if (error.message === 'This email has already been taken.') {
			...
			showCollectionFeesList(is_updated,had_removed,list){
				if(this.deposit_slip === true){
					if(is_updated !== true){
						axios.get(this.apiDomain + `/api/2022-fees-list-enabled-deposit-slip`)
						.then(res => {
			...
			addFileToUploadDepositList(f){
				this.show_collection_list = false
				var file = f;
				let DSimageUrl = null;
				let fee_id = this.fee_id
				this.validateFile(file.raw, "photo", (result) => {
					if(!result) {
						file = null;
					}
				...
				this.collection_list.map(function(item, index){
					if(item.id === fee_id){
						item['file_upload'] = f.raw
					}
				})
				this.showCollectionFeesList(true,false,this.collection_list)
			},
			saveDepositSlip(item) {
				this.fee_id = item.id;
			},
			removeDepositSlip(item){
				let fee_id = item.id
				this.collection_list.map(function(item, index){
            		if(item.id === fee_id){
						item.file_upload = "";
				...
				this.showCollectionFeesList(false,true,this.collection_list)
			}
		},
		mounted() {
