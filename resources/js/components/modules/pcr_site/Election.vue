		 ...
		 <el-dialog class="show-nominate-dialog" title="" :visible.sync="showSelectedNomineeDialog">
			...
			<section class="page-header">
				<div class="page-site-profile-header">
					<div class="page-site-election-header-text">MDMSI ELECTION</div>
				</div>
			</section>
				...
				<div class="container">
					<div class="row">
                        <div class="col-lg-12 mt-5 elec-header">
                            ...
                            <p class="mt-5 heading">Click on the picture or name of the candidate. You can only vote from 1 to 5 nominees.</p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-lg-12 mt-5 nominees justify-content-center">
							<el-checkbox-group v-model="selectedNominee">
								<div class="row justify-content-center">
									<div class="checkbox col-sm-12 col-md-4 col-lg-3" v-for="nominee in nomineeData" :key="nominee.id" :id="nominee.id">
										<el-checkbox :label="nominee.id" :v-for="nominee.id" @change="getCheck($event, nominee.id)">
					...
					<div class="row">
                        <div class="col-lg-12 mt-5 mb-5 vote-btn ">
							<el-button class="submit-vote-btn" type="primary" round @click="showSelectedNominee()">
								SUBMIT VOTE
							</el-button>
						</div>
					 </div>
...
export default {
    ...
    methods: {
		getCheck(value, id) {
			var index = this.selectedNominee.indexOf(id);
			let numVotes = this.selectedNominee.length
			if(numVotes <= this.maxVotes) {
				if(!value && index !== -1) { // For removing vote on previously nominated member
					$("#"+id).css('border','none');
					$("#"+id).removeClass("bordered");
				} else { // For adding votes
					if(value && index > -1) {
				...
				this.$message.error("You can only select up to 5 nominees")
				if (index !== -1) {
					this.selectedNominee.splice(index, 1);
				}
			}
		},
		fullname(item) {
		...
		getNomimeeDetails(){
            axios.get(this.apiDomain + '/api/election/nomineeDetails/'+this.activeElectionPeriod[0].id, {
                headers: { Authorization: "Bearer " + this.$cookies.get("token") }
            })
			.then(res => {
				if(res.data) {
		...
		showSelectedNominee() {
			if(this.selectedNominee.length == 0){
				...
				axios.put(this.apiDomain + `/api/election/selectedNominee`, data, {
								headers: {
		...
		submitVote(){
			...
			let data = {
           	...
           	axios.put(this.apiDomain + `/api/election/vote`, data, {
				headers: {
					Authorization: "Bearer " + this.$cookies.get("token"),
				},
			}).then(response => {
		...
		checkVoteStatus(election) {
			axios.get(this.apiDomain + '/api/election/checkVoteStatus/'+this.voterID+'/'+election, {
				headers: { Authorization: "Bearer " + this.$cookies.get("token"), }
			})
			.then((res) => {
				if(res.data){
		...
		getMember() {
			...
			.then((res) => {
				this.voterID = res.data.userable_id
				this.can_vote = res.data.member.can_vote
			...
			.catch((err) => {
				this.$message.error("Could not retrieve data, please contact the site admin")
				console.error(error)
			});
		},
    },
...
<style scoped>
	.page-header .page-site-profile-header {
		background-image: url(/images/layout/banner-election.jpg);
